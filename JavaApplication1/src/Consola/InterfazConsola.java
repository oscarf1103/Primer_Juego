...
public class InterfazConsola {
        ...
        while(!validado) {
            ...
            if(!(resp.length() > 1)) {
                if(resp.equalsIgnoreCase(opcion1) || resp.equalsIgnoreCase(opcion2)) {
                    ...
                    validado = false;
                    mostrarError("La información ingresada no es valido!");
                }
            } else {
                validado = false;
                mostrarError("Solo es permitido un caracter!");
            }
        }
    ...
    public int preguntarPosicion(String pregunta) {
        ...
        while(!validacion) {
            ...
            if (posicion < 0 || posicion > 7) {
                ...
                mostrarError("Solo se permite el uso de números");
                escanear.next();
                validacion = false;
            }
        }
    ...
    public int preguntarNumero(String pregunta) {
        ...
        while(!validacion) {
            ...
            if (posicion < 0) {
                validacion = false;
                mostrarError("Use Solo números positivos");
            }
            } catch (Exception e) {
